...
$author_more = function_exists( 'gyad_get_more_from_author' ) ? gyad_get_more_from_author( get_post( $post_id ), 4 ) : array();
$category_more = function_exists( 'gyad_get_more_from_category' ) ? gyad_get_more_from_category( get_post( $post_id ), 4 ) : array();

$author_ids = array();
foreach ( $author_more as $item ) {
	if ( $item instanceof WP_Post ) {
		$author_ids[] = (int) $item->ID;
	}
}

$category_more = array_values( array_filter( $category_more, function ( $item ) use ( $post_id, $author_ids ) {
	return $item instanceof WP_Post && (int) $item->ID !== (int) $post_id && ! in_array( (int) $item->ID, $author_ids, true );
} ) );
?>
...
<section class="article-continuation" aria-label="Continue reading">
	<div class="article-continuation__share">
		...
		<div class="article-continuation__share-actions" aria-label="Share this article again">
			...
			<button type="button" class="article-continuation__button" data-copy-url="<?php echo esc_attr( get_permalink( $post_id ) ); ?>"><span data-copy-label>Copy link</span></button>
		</div>
	</div>
	...
	<?php if ( ! empty( $author_more ) ) : ?>
		<section class="article-more-section" aria-labelledby="more-author-title">
			<div class="article-more-section__heading"><span class="article-more-section__eyebrow">From the author</span><h2 id="more-author-title">More from this author</h2></div>
			<div class="article-more-section__grid">
				<?php foreach ( $author_more as $item ) : ?>
					<?php global $post; $post = $item; setup_postdata( $post ); get_template_part( 'template-parts/archive/archive-card' ); ?>
				<?php endforeach; ?>
			</div>
		</section>
	...
	<?php if ( ! empty( $category_more ) ) : ?>
		<section class="article-more-section" aria-labelledby="more-category-title">
			<div class="article-more-section__heading"><span class="article-more-section__eyebrow">Keep exploring</span><h2 id="more-category-title">More in this section</h2></div>
			<div class="article-more-section__grid">

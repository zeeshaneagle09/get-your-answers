...
function gyad_asset_version( $relative_path ) {
	$file = GYAD_DIR . '/' . ltrim( $relative_path, '/' );
	return file_exists( $file ) ? (string) filemtime( $file ) : GYAD_VERSION;
}
...
function gyad_enqueue_assets() {
	...
	if ( is_author() ) {
		wp_enqueue_style( 'gyad-author-premium', GYAD_URI . '/assets/css/author-premium.css', array( 'gyad-cards' ), gyad_asset_version( 'assets/css/author-premium.css' ) );
	}
